#
#   Python MPEG Audio Information Module
#
...
class open_mp3:
       ...
       def __init__(self, file_to_open):
            if type(file_to_open) == type(''):
                try:
                    self.file = open(file_to_open, 'r+')
                except IOError:
                        self.file = open(file_to_open, 'r')
                        self.writable = 0
            else:
                 self.file = file_to_open
                 if self.file.mode == 'r+' or self.file.mode == 'w':
                     self.writable = 1
                 else:
                      self.writable = 0
       ...
       def __call__(self):
            ...
            try:
                ...
                comment = self.comment
                    ...
                    genre = songname = artist = album = year = comment = ''
       ...
       def write_tag(self):
            ...
            if self.file.read(3) != 'TAG':
                self.file.seek(-1, 2)
                self.file.write('TAG' + ' '*124 + '\377')
                self.file.seek(-125, 2)

            if self.set_songname != None:self.file.write(string.ljust(self.set_songname[:30], 30))
            else: self.file.seek(30, 1)
            if self.set_artist != None: self.file.write(string.ljust(self.set_artist[:30], 30))
            else: self.file.seek(30, 1)
            if self.set_album != None: self.file.write(string.ljust(self.set_album[:30], 30))
            else: self.file.seek(30, 1)
            if self.set_year != None: self.file.write(string.ljust(self.set_year[:4], 4))
            else: self.file.seek(4, 1)
            if self.set_comment != None: self.file.write(string.ljust(self.set_comment[:30], 30))
            else: self.file.seek(30, 1)
            if self.set_genre_no != None: 
	        if string.atoi(self.set_genre_no) > 255:
		    self.set_genre_no = '255'
	        self.file.write(chr(string.atoi(self.set_genre_no)))
            self.set_songname = self.set_artist = self.set_album = self.set_year = self.set_comment = self.set_genre = None
            self.filesize = self.filesize + 128
            self.read_tag()
            return 0
